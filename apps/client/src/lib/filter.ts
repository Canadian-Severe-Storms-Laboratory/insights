...
export function CardFilter(dataArray: productDataArray[], filterValues: any, userId: string = '') {
    ...
    const filteredData = parsedData.filter((data) => {
        const SearchStringCheck = data.name
            .toLowerCase()
            .includes(menuFilter.searchString.toLowerCase());
        const DateCheck =
            new Date(data.createdAt) >= menuFilter.startDate &&
            new Date(data.createdAt) <= menuFilter.endDate;
